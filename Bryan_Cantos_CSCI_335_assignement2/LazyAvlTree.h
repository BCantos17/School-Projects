...
// int removeSuccess( )   --> Return removeSuccess
...
{
  public:
    LazyAvlTree( ) : root{ nullptr }, insertCounter{ 0 }, removeCounter{ 0 }, 
                    removeSuccess { 0 }, containsCounter { 0 }, inTree{ false }
      { }
    
    LazyAvlTree( const LazyAvlTree & rhs ) : root{ nullptr }, insertCounter{ 0 }, removeCounter{ 0 }, 
                    removeSuccess { 0 }, containsCounter { 0 }, inTree{ false }
    {
        root = clone( rhs.root );
    }

    LazyAvlTree( LazyAvlTree && rhs ) : root{ rhs.root }, insertCounter{ 0 }, removeCounter{ 0 }, 
                    removeSuccess { 0 }, containsCounter { 0 }, inTree{ false }
    {
        rhs.root = nullptr;
    }
  ...
  private:
    ...
    mutable int insertCounter;
    mutable int removeCounter;
    mutable int removeSuccess;
    mutable int containsCounter;
    mutable bool inTree;
...
*****************************************************/
     ...
     */
    void printNode( const Comparable & x, AvlNode *t, ostream & out ) const
    {
        if( t == nullptr )
    ...
    /**
     * Internal method to get depth of tree
     */
    double sumDepth(AvlNode *t, int depthCounter) const
    {
        if (t == nullptr)
            return 0;
        else
